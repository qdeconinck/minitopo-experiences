...
def extractCellEnergyFromTrace(tracePath):
    cmd = ["cell_energy_estimator", tracePath, "outfile.out", "10.0.1.1"]
    if subprocess.call(cmd) != 0:
        print("Error when running cell_energy_estimator")
        return 0.0, 0.0
    ...
    for line in lines:
        if line.startswith("TOTAL;"):
            ...
            cell_energy, duration = float(split_line[1]), float(split_line[2])

    outfile.close()
    os.remove("outfile.out")
    return cell_energy, duration
    ...
    for filename in filenames:
        ...
        if filename.endswith("_client.pcap"):
            ...
            if "_mptcp_" in filename:
                ...
                cell_energy, duration = extractCellEnergyFromTrace(os.path.join(dirpath, filename))
            elif "_tcp_" in filename:
                wgetTime = extractTcpTimeFromTrace(os.path.join(dirpath, filename))
            if wgetTime is None:
                return
            dirpathSplit = dirpath.split("/")
            directory = dirpathSplit[-6]
            configInfo = dirpathSplit[-5]
            if timeInLabel:
                valuesCc.append({"value": (delays, percCell, timeUsedCell, startCell, cell_energy, duration), "label": configInfo + ": " + str(wgetTime) + "s", "directory": directory})
            else:
                valuesCc.append({"value": (delays, percCell, timeUsedCell, startCell, cell_energy, duration), "label": configInfo, "directory": directory})
...
def getSimpleTcpAndMptcpValues(results_directory="results", timeInLabel=True):
    ...
    for directory in os.listdir(results_directory):
        print(directory)
        if fnmatch.fnmatch(directory, 'sirimsg_*_mptcp'):
            for dirpath, dirnames, filenames in os.walk(os.path.join(results_directory, directory)):
                for filename in filenames:
                    # if filename == "https_client.log":
                    if filename.endswith("_client.pcap"):
                        topo = dirpath.split("/")[-5]
                        if topo not in mptcpValues:
                            mptcpValues[topo] = {}

                        scheduler = dirpath.split("/")[-4]
                        ...
                        use_fastjoin = dirpath.split("/")[-2]
                        if use_fastjoin not in mptcpValues[topo][scheduler][open_bup]:
                            mptcpValues[topo][scheduler][open_bup][use_fastjoin] = {}

                        congestion_control = dirpath.split("/")[-1]
                        if congestion_control not in mptcpValues[topo][scheduler][open_bup][use_fastjoin]:
                            mptcpValues[topo][scheduler][open_bup][use_fastjoin][congestion_control] = []

                        getSimpleValuesInCc(mptcpValues[topo][scheduler][open_bup][use_fastjoin][congestion_control], dirpath, filenames, timeInLabel=timeInLabel)
...
def boxPlot(mptcpValues, output):
    text = {("default-1_server", "0", "0"): "Oracle",
            # ("default-1_server", "0", "1"): "Oracle with Fast Join",
            ("default-1_server", "1", "0"): "No oracle"}
    results = {}
    ...
    for topo in mptcpValues:
        ...
        labels = []
        results[topo] = {}
        for config in [("default-1_server", "1", "0"), ("default-1_server", "0", "0")]:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output, 'oracle_sirimsg_delays.pdf'))
    plt.close('all')
    ...
    # hL.set_visible(False)
    plt.tight_layout()
    plt.savefig(os.path.join(output, 'oracle_sirimsg_perc_cell.pdf'))
    plt.close('all')
    ...
    # hL.set_visible(False)
    plt.tight_layout()
    plt.savefig(os.path.join(output, 'oracle_sirimsg_time_used.pdf'))
    plt.close('all')
    ...
    plt.tick_params(axis='both', which='minor', labelsize=26, pad=15)
    plt.tight_layout()
    plt.savefig(os.path.join(output, 'oracle_sirimsg_start_cell.pdf'))
    plt.close('all')


def linePlot(mptcpValues, output):
    configs = [
        ("1", "No oracle", "blue", "o", '-'),
        # ("0-100", "Oracle - sloss_threshold 10 %", "green", "v", "--"),
        ("0", "Oracle - sloss_threshold 25 %", "orange", "D", "-."),
        # ("0-400", "Oracle - sloss_threshold 40 %", "#73D7D7", "^", ":"),
    ]
    topos = [
        ("0_d7.5b10_1_d12.5b10_nt_0_0_loss_0%_nt_1_0_loss_0%", 0),
        ("0_d7.5b10_1_d12.5b10_nt_0_0_loss_0%_nt_1_0_loss_0%_nt_0_7_loss_5%", 5),
        ("0_d7.5b10_1_d12.5b10_nt_0_0_loss_0%_nt_1_0_loss_0%_nt_0_7_loss_10%", 10),
        ("0_d7.5b10_1_d12.5b10_nt_0_0_loss_0%_nt_1_0_loss_0%_nt_0_7_loss_15%", 15),
        ("0_d7.5b10_1_d12.5b10_nt_0_0_loss_0%_nt_1_0_loss_0%_nt_0_7_loss_20%", 20),
        ("0_d7.5b10_1_d12.5b10_nt_0_0_loss_0%_nt_1_0_loss_0%_nt_0_7_loss_25%", 25),
        ("0_d7.5b10_1_d12.5b10_nt_0_0_loss_0%_nt_1_0_loss_0%_nt_0_7_loss_30%", 30),
        ("0_d7.5b10_1_d12.5b10_nt_0_0_loss_0%_nt_1_0_loss_0%_nt_0_7_loss_40%", 40),
        ("0_d7.5b10_1_d12.5b10_nt_0_0_loss_0%_nt_1_0_loss_0%_nt_0_7_loss_50%", 50),
        ("0_d7.5b10_1_d12.5b10_nt_0_0_loss_0%_nt_1_0_loss_0%_nt_0_7_loss_100%", 100),
    ]
    ...
    for openBup, configText, color, marker, ls in configs:
        x = []
        y = []
        ymin = []
        ymax = []
        ...
        for topo, x_val in topos:
            delays = []
            for item in mptcpValues[topo]["default-1_server"][openBup]["0"]["olia"]:
                delays += item["value"][0]

            med_delay = np.median(delays)
            # std_delay = np.std(delays)
            # min_delay = np.min(delays)
            min_delay = np.percentile(delays, 25)
            # max_delay = np.max(delays)
            max_delay = np.percentile(delays, 75)
            x.append(x_val)
            y.append(med_delay)
    ...
    plt.xlabel("Random losses (%)", fontsize=30)
    plt.ylabel("Max Delay [ms]", fontsize=30)
    plt.tick_params(axis='both', which='major', labelsize=30, pad=15)
    plt.tick_params(axis='both', which='minor', labelsize=26, pad=15)
    ...
    # plt.xscale("symlog")
    plt.tight_layout()
    plt.savefig(os.path.join(output, 'oracle_sirimsg_lines_loss.png'), transparent=True)
    # plt.savefig(os.path.join(output, 'oracle_sirimsg_lines_loss.pdf'))
    ...
    for openBup, configText, color, marker, ls in configs:
        ...
        for topo, x_val in topos:
            cell_powers = []
            for item in mptcpValues[topo]["default-1_server"][openBup]["0"]["olia"]:
                cell_powers.append(item["value"][4] / item["value"][5] * 1000.0)

            med_cell_power = np.median(cell_powers)
            # std_delay = np.std(delays)
            # min_delay = np.min(delays)
            min_cell_power = np.percentile(cell_powers, 25)
            # max_delay = np.max(delays)
            max_cell_power = np.percentile(cell_powers, 75)
            x.append(x_val)
            y.append(med_cell_power)
            ymin.append(med_cell_power - min_cell_power)
            ymax.append(max_cell_power - med_cell_power)
    ...
    plt.xlabel("Random losses (%)", fontsize=30)
    plt.ylabel("Mean Cell Power [mW]", fontsize=28)
    plt.tick_params(axis='both', which='major', labelsize=30, pad=15)
    plt.tick_params(axis='both', which='minor', labelsize=26, pad=15)
    ...
    # plt.xscale("symlog")
    plt.tight_layout()
    plt.savefig(os.path.join(output, 'oracle_sirimsg_lines_loss_cell_power.png'), transparent=True)
    # plt.savefig(os.path.join(output, 'oracle_sirimsg_lines_loss_cell_power.pdf'))
