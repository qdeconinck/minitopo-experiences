...
def boxPlot(results, output):
    tcpValues, mptcpValues = getSimpleTcpAndMptcpValues(results_directory=results, timeInLabel=False)
...
def linePlot(mptcpValues, output_dir):
    configs = [
        ("0-t1", "1 ms", "blue", "o", "-"),
        ("0-t10", "10 ms", "orange", "v", "--"),
        ("0-t100", "100 ms", "#73D7D7", "D", "-."),
        ("0-t500", "500 ms", "green", "^", ":"),
        ("0-t1000", "1000 ms", "purple", "8", "-"),
    ]
    topos = [
    ...
    for timer, timerText, color, marker, ls in configs:
        x = []
        y = []
        ymin = []
        ymax = []
        ...
        for topo, x_val in topos:
            ...
            for item in mptcpValues[topo]["default-1_server"][timer]["1"]["olia"]:
                delays_1 += item["value"][0]

            med_delay = np.median(delays_1)
            # std_delay = np.std(delays)
            # min_delay = np.min(delays)
            min_delay = np.percentile(delays_1, 25)
            # max_delay = np.max(delays)
            max_delay = np.percentile(delays_1, 75)
            x.append(x_val)
            y.append(med_delay)
        ...
        print(ymin, ymax)
        eb1 = ax.errorbar(x, y, yerr=[ymin, ymax], color=color, label=timerText, linewidth=2, marker=marker, linestyle=ls)
    ...
    plt.xlim(-1, 101)
    plt.ylim(ymax=2000)
    ax.legend(loc="best", ncol=3, fontsize=24)
    # plt.yscale("log")
    # plt.xscale("symlog")
    plt.grid()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'oracle_timer_siri_max_delay.pdf'))
